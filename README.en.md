...
| pty        | (browser) /pty.html       | —                              | beta     | Interactive PTY terminal (xterm.js + WebSocket). ssh = asyncssh PTY channel, local = Unix pty.openpty(). ConPTY (Windows-local) and HTTP PTY are deferred to M3.5 |
